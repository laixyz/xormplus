...
func TestPingContext(t *testing.T) {
	...
	err := testEngine.(*xormplus.Engine).PingContext(ctx)
	assert.Error(t, err)
...
func TestAutoTransaction(t *testing.T) {
	...
	engine := testEngine.(*xormplus.Engine)

	// will success
	engine.Transaction(func(session *xormplus.Session) (interface{}, error) {
	...
	// will rollback
	_, err = engine.Transaction(func(session *xormplus.Session) (interface{}, error) {
...
func TestDumpTables(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.NoError(t, testEngine.(*xormplus.Engine).DumpTablesToFile([]*schemas.Table{tb}, fp))
	...
	for _, tp := range []schemas.DBType{schemas.SQLITE, schemas.MYSQL, schemas.POSTGRES, schemas.MSSQL} {
		...
		t.Run(name, func(t *testing.T) {
			assert.NoError(t, testEngine.(*xormplus.Engine).DumpTablesToFile([]*schemas.Table{tb}, name, tp))
		})
	}
}
