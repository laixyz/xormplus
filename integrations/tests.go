...
var (
	testEngine xormplus.EngineInterface
	dbType     string
	connString string
...
func createEngine(dbType, connStr string) error {
	if testEngine == nil {
		...
		if !*cluster {
			...
			testEngine, err = xormplus.NewEngine(dbType, connStr)
		} else {
			testEngine, err = xormplus.NewEngineGroup(dbType, strings.Split(connStr, *splitter))
