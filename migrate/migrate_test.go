...
var (
	migrations = []*Migration{
		{
			ID: "201608301400",
			Migrate: func(tx *xormplus.Engine) error {
				return tx.Sync2(&Person{})
			},
			Rollback: func(tx *xormplus.Engine) error {
...
func TestMigration(t *testing.T) {
	...
	db, err := xormplus.NewEngine("sqlite3", dbName)
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
...
func TestInitSchema(t *testing.T) {
	...
	m := New(db, DefaultOptions, migrations)
	m.InitSchema(func(tx *xormplus.Engine) error {
...
func tableCount(db *xormplus.Engine, tableName string) (count int) {
